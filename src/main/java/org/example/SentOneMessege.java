...
//        }
        if (!sentArticlesCommunity.contains(article)) {
            ...
            if (percent >= 0.5 || (Double.parseDouble(itemfFeedBackCost) - Double.parseDouble(itemCost) >= 199 && percent > 1)) {
                boolean bol = hasFeedbackPoints(article, Cookies);
                if (!bol) {
                    return;
                }
    ...
    String createMessege(String itemName, String itemCost, String itemfFeedBackCost, String article, Double percent,String totalQuery){
